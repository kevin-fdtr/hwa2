        ...
        _data.read('users', queryString.data.emailAddress, (err, userData) => {
          if (!err && userData) {
            _data.read('carts', queryString.data.emailAddress, (err, cartData) => {
              if (!err) {
                _data.delete('carts', queryString.data.emailAddress, (err) => {
                  if (!err) {
                    callback(200, cartData);
                  } else {
                    callback(500, {'Error': 'User\'s cart was not deleted.'});
                  }
                })
              } else {
                callback(400, {'Error': 'User\'s cart could not be read. Cart may not exist.'});    
              }
            });
          } else {
